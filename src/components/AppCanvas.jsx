...
function AppCanvas(props) {
  ...
  const loadModelButtons = (buttons) => {
    setModelButtons(buttons)
  }

  const loadExtModel = (button) => {
    const pathUrl = new URL(button.model, import.meta.url).href
    fetch(pathUrl).then(response => {
      if (response.ok) {
        return response.json()
      }
    }).then(data => {
      JEELIZVTOWIDGET.load_modelStandalone(data);
    })
  }

  window.loadModelButtons = loadModelButtons
